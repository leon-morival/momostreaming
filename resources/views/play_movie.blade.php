...
@section('content')
    <div class="container mx-auto mt-4 text-white">
        ...
        <div class="flex flex-wrap mt-4">
            ...
            <div class="w-full md:w-7/12 md:px-2 md:pt-0 px-9 ">
                <div class="relative" style="padding-top: 56.25%;"> <!-- 16:9 Aspect Ratio -->
                    <iframe id="moviePlayer" class="absolute top-0 left-0 w-full h-full" src="" frameborder="0"
            ...
            <div class="w-full md:w-1/4 pb-5 lg:pb-0 ">
                {{-- Movie Information --}}
                <div>
            ...
            fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${API_KEY}`)
                ...
                .then(data => {
                    if (data) {
                        ...
                        document.getElementById('movieImage').src = data.poster_path ? IMG_URL + data
                            .poster_path : 'http://via.placeholder.com/1080x1580';
                        document.getElementById('movieTitleInfo').textContent = data.title;
                        document.getElementById('rated').textContent = data.adult ? 'R' : 'PG';
                        document.getElementById('movieYear').textContent = new Date(data.release_date)
                            .getFullYear();
                        ...
                        document.getElementById('movieRating').textContent = data.vote_average ? data
                        ...
                        fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits?api_key=${API_KEY}`)
                            ...
                            .then(credits => {
                                ...
                                credits.crew.filter(member => member.job === 'Director').forEach(
                                    director => {
                                        const li = document.createElement('li');
                                        li.classList.add('flex', 'items-center', 'mb-2');
                                        const img = document.createElement('img');
                                        img.src = director.profile_path ? IMG_URL + director
                                            .profile_path : 'http://via.placeholder.com/50x50';
